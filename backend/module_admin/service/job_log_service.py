...
class JobLogService:
    ...
    @classmethod
    def update_job_log_services(
        cls, query_db: Session, job_id: int, start_time: datetime, update_data: dict
    ):
        """
        更新定时任务日志信息service
        ...
        """
        try:
            updated_rows = JobLogDao.update_job_log_by_job_id_and_start_time(
                query_db, job_id, start_time, update_data
            )
            query_db.commit()
            result = dict(is_success=updated_rows > 0, message='更新成功' if updated_rows > 0 else '未找到日志记录')
        except Exception as e:
            query_db.rollback()
            result = dict(is_success=False, message=str(e))
    ...
    @classmethod
    def update_job_log_by_id_services(
        cls, query_db: Session, job_log_id: int, update_data: dict
    ):
        """
        根据任务日志主键 ID 更新日志
        ...
        """
        try:
            updated_rows = JobLogDao.update_job_log_by_id(
                query_db, job_log_id, update_data
            )
            query_db.commit()
            result = dict(is_success=updated_rows > 0, message='更新成功' if updated_rows > 0 else '未找到日志记录')
        except Exception as e:
