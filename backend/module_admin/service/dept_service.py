...
class DeptService:
    ...
    async def add_dept_services(cls, query_db: AsyncSession, page_object: DeptModel):
        ...
        parent_id = page_object.parent_id or 0
        if parent_id == 0:
            page_object.ancestors = '0'
        else:
            parent_info = await DeptDao.get_dept_by_id(query_db, parent_id)
            if not parent_info:
            ...
            page_object.ancestors = f'{parent_info.ancestors},{parent_id}'
        try:
    ...
    async def edit_dept_services(cls, query_db: AsyncSession, page_object: DeptModel):
        ...
        parent_id = page_object.parent_id or 0
        new_parent_dept = await DeptDao.get_dept_by_id(query_db, parent_id) if parent_id else None
        old_dept = await DeptDao.get_dept_by_id(query_db, page_object.dept_id)
        try:
            if parent_id == 0:
                page_object.ancestors = '0'
                if old_dept:
                    await cls.update_dept_children(query_db, page_object.dept_id, '0', old_dept.ancestors or '')
            elif new_parent_dept and old_dept:
                new_ancestors = f'{new_parent_dept.ancestors},{new_parent_dept.dept_id}'
                old_ancestors = old_dept.ancestors or ''
