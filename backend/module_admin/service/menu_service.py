...
class MenuService:
    ...
    def list_to_tree(cls, permission_list: list) -> list:
        ...
        permission_list = [
            {
                ...
                'query': item.query or '',
                # 确保路由名称不为空，如果为空则使用菜单名称作为路由名称
                'routeName': item.route_name if item.route_name and item.route_name.strip() else item.menu_name,
        ...
        # 转成id为key的字典
        mapping: dict = dict(zip([i['menuId'] for i in permission_list], permission_list))
        ...
        def sort_menus(menus):
            ...
            menus.sort(key=lambda x: x.get('orderNum', 0))
        
        sort_menus(container)
        return container
