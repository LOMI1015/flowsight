...
class ServerService:
    ...
    async def get_server_monitor_info():
        ...
        for i in io:
            try:
                # 使用 mountpoint 而不是 device，因为 mountpoint 更可靠
                # 在容器环境中，某些设备文件可能不存在
                disk_path = i.mountpoint if i.mountpoint else i.device
                o = psutil.disk_usage(disk_path)
                disk_data = SysFiles(
                    dirName=i.device,
                ...
                # 跳过无法访问的磁盘分区（在容器环境中很常见）
                # 例如：/dev/vda3 等设备文件可能不存在
                from utils.log_util import logger
                logger.warning(f'无法访问磁盘分区 {i.device} ({i.mountpoint}): {str(e)}')
                continue
