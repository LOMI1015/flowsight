...
class UserService:
    ...
    async def get_user_list_services(
        ...
        if is_page:
            # 处理用户岗位和角色信息，将同一用户的多个岗位和角色合并
            user_dict = {}
            for row in query_result.rows:
                # row 是一个包含用户、部门、岗位、角色信息的列表，已经被CamelCaseUtil转换
                if isinstance(row, list) and len(row) >= 4:
                    user_data = row[0] if isinstance(row[0], dict) else {}
                    dept_data = row[1] if isinstance(row[1], dict) else None
                    post_data = row[2] if isinstance(row[2], dict) else None
                    role_data = row[3] if isinstance(row[3], dict) else None
                else:
                    # 如果数据结构不是预期的，跳过
                    continue
                ...
                # 如果有岗位信息，添加到用户岗位列表中（去重）
                if post_data and (post_data.get('postId') or post_data.get('post_id')):
                    post_id = post_data.get('postId') or post_data.get('post_id')
                    # 检查是否已存在相同的岗位
                    existing_post = next((p for p in user_dict[user_id]['userPosts'] if p['postId'] == post_id), None)
                    if not existing_post:
                        user_dict[user_id]['userPosts'].append({
                            'postId': post_id,
                            'postName': post_data.get('postName') or post_data.get('post_name'),
                            'postCode': post_data.get('postCode') or post_data.get('post_code')
                        })
                # 如果有角色信息，添加到用户角色列表中（去重）
                if role_data and (role_data.get('roleId') or role_data.get('role_id')):
                    role_id = role_data.get('roleId') or role_data.get('role_id')
                    # 检查是否已存在相同的角色
                    existing_role = next((r for r in user_dict[user_id]['userRoles'] if r['roleId'] == role_id), None)
                    if not existing_role:
                        user_dict[user_id]['userRoles'].append({
                            'roleId': role_id,
                            'roleName': role_data.get('roleName') or role_data.get('role_name'),
                            'roleKey': role_data.get('roleKey') or role_data.get('role_key')
                        })
            
            user_list_result = PageResponseModel(
                **{
                    **query_result.model_dump(by_alias=True),
                    'rows': list(user_dict.values()),
                }
            )
        else:
            user_list_result = []
            if query_result:
                # 处理用户岗位信息，将同一用户的多个岗位合并
                user_dict = {}
                for row in query_result:
                    # row 是一个包含用户、部门、岗位信息的列表，已经被CamelCaseUtil转换
                ...
                user_list_result = list(user_dict.values())
    ...
    async def add_user_services(cls, query_db: AsyncSession, page_object: AddUserModel):
        ...
        if not await cls.check_user_name_unique_services(query_db, page_object):
            ...
            try:
                # 设置默认头像
                if not add_user.avatar:
                    add_user.avatar = 'boy-purple.png'
                # 设置默认密码：用户名 + 123
                default_password = f"{page_object.user_name}123"
                add_user.password = PwdUtil.get_password_hash(default_password)
                add_result = await UserDao.add_user_dao(query_db, add_user)
                user_id = add_result.user_id
                if hasattr(page_object, 'role_ids') and page_object.role_ids:
                    for role in page_object.role_ids:
                        await UserDao.add_user_role_dao(query_db, UserRoleModel(userId=user_id, roleId=role))
                if hasattr(page_object, 'post_ids') and page_object.post_ids:
        ...
        try:
            for index, row in df.iterrows():
                ...
                add_user = UserModel(
                    ...
                    sex=row['sex'],
                    status=row['status'],
                    avatar='http://8.134.192.226:9000/artwork/sh_def_man.png',  # 设置默认头像
    ...
    async def add_user_role_services(cls, query_db: AsyncSession, page_object: CrudUserRoleModel):
        ...
        :return: 新增用户关联角色校验结果
        """
        if page_object.user_id and hasattr(page_object, 'role_ids') and page_object.role_ids:
            ...
            try:
                ...
                await query_db.rollback()
                raise e
        elif page_object.user_id and (not hasattr(page_object, 'role_ids') or not page_object.role_ids):
            try:
                await UserDao.delete_user_role_by_user_and_role_dao(query_db, UserRoleModel(userId=page_object.user_id))
                await query_db.commit()
                return CrudResponseModel(is_success=True, message='分配成功')
