...
class LoginService:
    ...
    async def get_current_user_routers(cls, user_id: int, query_db: AsyncSession):
        ...
        cls.__ensure_user_center_route(user_router)
    ...
    @classmethod
    def __ensure_user_center_route(cls, router_list: List[RouterModel]) -> None:
        """
        确保个人中心路由存在，但不生成菜单（所有角色可见）
        """
        if cls.__router_path_exists(router_list, '/user-center'):
            return
        router_list.append(
            RouterModel(
                hidden=True,
                ...
                meta=MetaModel(title='个人中心', icon='ri:user-3-line', noCache=False),
            )
        )

    @classmethod
    def __router_path_exists(cls, routers: List[RouterModel], target_path: str) -> bool:
        normalized_target = target_path.lstrip('/')
        for router in routers:
            if router.path and router.path.lstrip('/') == normalized_target:
                return True
            if router.children and cls.__router_path_exists(router.children, target_path):
                return True
        return False
