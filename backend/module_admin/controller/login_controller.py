...
async def login(
    ...
    return ResponseUtil.success(msg='登录成功', data={'access_token': access_token})
...
async def get_login_user_info(
    ...
    logger.info('获取成功')
    if current_user.user and getattr(current_user.user, 'avatar', None):
        current_user.user.avatar = _build_avatar_url(request, current_user.user.avatar)
